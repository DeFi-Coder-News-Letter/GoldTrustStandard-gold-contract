...
contract('BetaDelegatedTransfer GTSG', function ([_, admin, owner, executor, recipient, whitelister, bystander]) {
  ...
  describe('as a token with delegated transfer', function () {
    beforeEach(async function () {
      ...
      let {receipt} = await this.token.transfer(fromAddress, 2 * (amount + serviceFeeAmount), {from: owner});
    ...
    it('can do a delegated transfer', async function () {
      ...
      // try replays
      await assertRevert(
        this.token.betaDelegatedTransfer(sig, to, value, serviceFee, 0, deadline, {from: executor})
      );
      await assertRevert(
        this.token.betaDelegatedTransfer(sig, to, value, serviceFee, 1, deadline, {from: executor})
      );
    });
    ...
    describe('with multiple delegated transfers', function () {
      ...
      it('can do two delegated transfers', async function () {
        ...
        await this.token.betaDelegatedTransfer(this.sig, to, value, serviceFee, 0, deadline, {from: executor});
        // commit second delegated transfer
        await this.token.betaDelegatedTransfer(this.sig2, to, value, serviceFee, 1, deadline, {from: executor});
        ...
        // try replays
        await assertRevert(
          this.token.betaDelegatedTransfer(this.sig, to, value, serviceFee, 0, deadline, {from: executor})
        );
        await assertRevert(
          this.token.betaDelegatedTransfer(this.sig, to, value, serviceFee, 1, deadline, {from: executor})
        );
        await assertRevert(
          this.token.betaDelegatedTransfer(this.sig, to, value, serviceFee, 2, deadline, {from: executor})
        );
      });
      ...
      it('can do two delegated transfers in a batch', async function () {
        ...
        await this.token.betaDelegatedTransferBatch(rs, ss, vs, tos, values, serviceFees, seqs, deadlines, {from: executor});
      ...
      it('reverts batches to act atomically', async function () {
        ...
        // commit delegated transfers in batch
        await assertRevert(
          this.token.betaDelegatedTransferBatch(rs, ss, vs, tos, values, serviceFees, seqs, deadlines, {from: executor})
        );
    ...
    it('fails for bad signatures', async function () {
      ...
      const wrongSig = ethSigUtil.signTypedData(wrongPrivateKey, {data: typedData});
      let {to, value, serviceFee, seq, deadline} = message;
      // sig too long
      let badSig = sig + [0x71];
      assert.equal(sig.length + 3, badSig.length);
      await assertRevert(
        this.token.betaDelegatedTransfer(wrongSig, to, value, serviceFee, seq, deadline, {from: executor})
      );
      await assertRevert(
          this.token.betaDelegatedTransfer(badSig, to, value, serviceFee, seq, deadline, {from: executor})
      );
    });
    ...
    it('fails for bad seq', async function () {
      ...
      await assertRevert(
        this.token.betaDelegatedTransfer(sig, to, value, serviceFee, badSeq, deadline, {from: executor})
      );
    });
    ...
    it('fails for insufficient balance', async function () {
      ...
      let {to, value, serviceFee, seq, deadline} = message;
      await assertRevert(
        this.token.betaDelegatedTransfer(sig, to, value, serviceFee, seq, deadline, {from: executor})
      );
    });
    ...
    it('fails for a non-whitelisted address', async function () {
      ...
      let {to, value, serviceFee, seq, deadline} = message;
      await assertRevert(
        this.token.betaDelegatedTransfer(sig, to, value, serviceFee, seq, deadline, {from: owner})
      );
    });
